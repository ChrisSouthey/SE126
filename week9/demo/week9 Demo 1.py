...
def swap(listName, posi):
    ...
    #return nothng: because lists nare stored to memory for the program; when we swap value places, list is automatically updated
...
#1 - connect to a file and import its data into lists
with open("week9/demo/finalReview_text.csv") as csvfile:
...
avg_age = total_age / len(age) #calculate average age
...
print(f"\n\nOldest Person: {old_fName} is {old_age}yrs old, ")
print(f"\n\nYoungest Person: {young_fName} is {young_age}yrs old, ")
...
while answer == "y":
    ...
    if userChoice == 1:
        ...
        #create an emoty list to store found values
        found = []
        ...
        if not found: #checks to see if list is empty
            ...
            #we found at least 1 person
            print(f"Heres what we found for {search}")
    ...
    if userChoice == 2:
        ...
        #print results
        if search == firstName[mid]:
            print(f"\n\tWe found your search for: {search}")
            ...
            print(f"We could not find {search} :(")
